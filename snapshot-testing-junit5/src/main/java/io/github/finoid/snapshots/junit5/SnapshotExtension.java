...
import io.github.finoid.snapshots.config.SnapshotConfigInjector;
...
public class SnapshotExtension
    ...
    private boolean shouldFailOnOrphans(ExtensionContext context) {
        try {
            ...
            if (testDescriptor instanceof ClassTestDescriptor classTestDescriptor) { // Junit 5.3.2
                return classTestDescriptor.getChildren().size() > 1;
            } else if (testDescriptor instanceof ClassBasedTestDescriptor classTestDescriptor) { // Junit 5.7.2
                return classTestDescriptor.getChildren().size() > 1;
            }
        } catch (Exception e) {
            log.error(
